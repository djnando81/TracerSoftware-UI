    ...
    colRol.setCellValueFactory(new PropertyValueFactory<>("rol"));
        ...
        colActivo.setCellFactory(tc -> new javafx.scene.control.TableCell<UsuarioDTO, Boolean>() {
            ...
            private final javafx.scene.Node inlineLoader;
            {
                ...
                toggle.getStyleClass().add("cell-toggle-btn");
                ...
                stack.getChildren().addAll(toggle, inlineLoader);
                stack.setPrefWidth(javafx.scene.layout.Region.USE_COMPUTED_SIZE);
                stack.setMaxWidth(javafx.scene.layout.Region.USE_COMPUTED_SIZE);
                javafx.scene.layout.StackPane.setAlignment(toggle, javafx.geometry.Pos.CENTER);
                javafx.scene.layout.StackPane.setAlignment(inlineLoader, javafx.geometry.Pos.CENTER_RIGHT);

                // action handler: update model + backend with overlay/loader
                toggle.setOnAction(e -> {
                    ...
                    // set immediate visual feedback via CSS classes
                    toggle.getStyleClass().setAll("cell-toggle-btn", newState ? "toggle-activo" : "toggle-inactivo");
                    toggle.setText(newState ? "Activo" : "Inactivo");
                    ...
                    Task<Void> t = new Task<>() {
                        ...
                        protected Void call() throws Exception {
                            ...
                            dto.put("activo", newState);
                            // Ensure required simple fields are present (some backends require nombre/email on update)
                            try {
                                if ((!dto.has("nombre") || dto.path("nombre").asText().isEmpty()) && u.getNombre() != null) {
                                    dto.put("nombre", u.getNombre());
                                }
                                if ((!dto.has("email") || dto.path("email").asText().isEmpty()) && u.getEmail() != null) {
                            ...
                            service.updateUser(u.getId(), root);
                            return null;
                        }
                    };
                    t.setOnSucceeded(ev -> {
                    ...
                    t.setOnFailed(ev -> {
                        ...
                        toggle.setSelected(u.isActivo());
                        toggle.setText(u.isActivo() ? "Activo" : "Inactivo");
                        toggle.getStyleClass().setAll("cell-toggle-btn", u.isActivo() ? "toggle-activo" : "toggle-inactivo");
                        toggle.setDisable(false);
                        hideRowOverlay(idx);
                                ...
                                if (res.isPresent() && res.get() != null && !res.get().isEmpty()) {
                                    ...
                                    Task<Void> retry = new Task<>() {
                                        ...
                                        protected Void call() throws Exception {
                                            ...
                                            service.updateUser(u.getId(), root2);
                                            return null;
                                        }
                                    };
            ...
            protected void updateItem(Boolean item, boolean empty) {
                ...
                try {
                    javafx.scene.control.TableRow<?> row = getTableRow();
                    if (row != null) toggle.prefHeightProperty().bind(row.heightProperty());
                } catch (Exception ignored) {}

                boolean sel = u.isActivo();

                toggle.setSelected(sel);
                toggle.setText(sel ? "Activo" : "Inactivo");
                toggle.getStyleClass().setAll("cell-toggle-btn", sel ? "toggle-activo" : "toggle-inactivo");

                inlineLoader.visibleProperty().bind(toggle.disableProperty());
