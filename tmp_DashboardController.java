...
public class DashboardController {
    ...
    public void initialize() {
        // Establecer texto con sÃ­mbolo monetario desde Java para evitar parsing en FXML
        if (salesLabel != null) {
        ...
        // Montar menÃº lateral aditivo desde manifest JSON
        try {
        ...
        DefaultCurrentUserProvider userProvider = new DefaultCurrentUserProvider(
            ...
            VBox sidebar = builder.build(manifest, userProvider,
                    // navigate callback -> intentarÃ¡ cargar /fxml/{routeName}.fxml y marcar botÃ³n activo
                    route -> {
                        try {
                    ...
                    action -> {
                        ...
                        if ("openSettings".equals(action)) {
                            MessageToast.show(null, "Ajustes no implementados aÃºn.", MessageToast.ToastType.INFO);
                        }
                    }
            );
            ...
            // Enlazar botones estÃ¡ticos a la misma navegaciÃ³n que usa el sidebar (rutas del manifest)
            try {
    ...
    public void handleLogout() {
        // AquÃ­ puedes agregar lÃ³gica para cerrar sesiÃ³n, volver al login, etc.
        System.out.println("Logout ejecutado desde el botÃ³n âŽ‹");
    }
